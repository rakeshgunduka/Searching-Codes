...
void print_visited(){
	...
	printf("\n");
	for(i = 0; i<vs ;i++ ){
		printf("h[%d] = %d\tg[%d] = %d\n",visited[i],hf[visited[i]],visited[i],gf[visited[i]]);
	}
	printf("\n");
}
...
int find_low_path(int *hc,int i,int j){
	...
	if((hf[hc[rmin]] + gf[hc[rmin]]) < (hc[hc[lmin]] + gf[hc[lmin]])){
		return rmin;
	}else{
		return lmin;
	}
...
void Astar_search(int row,int start){
	int i = row,j,m;
	int ret;
	for(j = start;j<n;j++){
		if(conn[i][j] == 1){
